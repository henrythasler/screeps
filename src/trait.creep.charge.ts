...
export function check(creep: Creep): Task {
    if ((creep.memory.task != Task.CHARGE) && (nonInterruptableTasks.indexOf(creep.memory.task) < 0) && (creep.store[RESOURCE_ENERGY] < 10) ||
        ...
        if (creep.store.getFreeCapacity() > 0) {
            ...
            if (container.length > 0) {
                ...
                if (creep.withdraw(container[0], RESOURCE_ENERGY) == ERR_NOT_IN_RANGE) {
                    ...
                    creep.memory.lastChargeSource = EnergySource.CONTAINER;
                }
                return creep.memory.task;
            }
            ...
            if (sources.length > 0) {
                ...
                if (creep.harvest(sources[sourceId]) == ERR_NOT_IN_RANGE) {
                    ...
                    creep.memory.lastChargeSource = EnergySource.SOURCE;
                }
                return creep.memory.task;
            }
        }
